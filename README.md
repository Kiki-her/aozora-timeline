...
アプリで使用している青空文庫のデータは、初回起動時にローカルに保存されます。
最新のデータを取得したい場合や、URLが古い形式になっている場合は、以下の手順でデータを再取得できます：

1. アプリを起動
2. 画面下部のナビゲーションから「設定」（⚙️アイコン）をタップ
3. 「データ再取得」をタップ
4. データが更新され、最新の青空 in Browsers URLが適用されます

**注意**: データ再取得を行うと、すべての書籍データが最新のCSVデータで上書きされます。いいね・読了の記録は保持されます。
